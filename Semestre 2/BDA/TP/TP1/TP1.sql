...
SELECT DISTINCT instrument FROM enregistrement INNER JOIN interprete ON interprete.id_enr = enregistrement.id_enr WHERE annee_enr BETWEEN date '1980-01-01' AND date '1990-01-01';

--Question 7

SELECT id_cd FROM enregistrement WHERE duree_enr = (SELECT MAX(duree_enr) FROM enregistrement WHERE cond_enr IN ('public', 'acoustic'));

--Question 8

SELECT cd.id_cd AS "ID du cd", titre_cd AS "Titre du cd", SUM(duree_enr) AS "Durée du cd" FROM cd INNER JOIN enregistrement ON cd.id_cd = enregistrement.id_cd GROUP BY titre_cd, cd.id_cd ORDER BY cd.id_cd ASC;

--Question 9

SELECT nom_pers AS "Introducteur du bandonéon", annee_enr AS "Première année d'apparition" FROM interprete INNER JOIN enregistrement ON enregistrement.id_enr = interprete.id_enr INNER JOIN personne ON id_interprete = id_pers WHERE instrument='bandoneon' ORDER BY annee_enr ASC LIMIT 1 OFFSET 0;
...
SELECT nom_pers AS nom, prenom_pers AS prénom, age(MAX(annee_enr), MIN(annee_enr)) AS "Durée de carrière" FROM interprete INNER JOIN enregistrement ON (interprete.id_cd, interprete.id_enr) = (enregistrement.id_cd, enregistrement.id_enr) INNER JOIN personne ON id_pers = id_interprete GROUP BY nom_pers, prenom_pers HAVING age(MAX(annee_enr), MIN(annee_enr)) >= ALL (SELECT age(MAX(annee_enr), MIN(annee_enr)) FROM interprete INNER JOIN enregistrement ON (interprete.id_cd, interprete.id_enr) = (enregistrement.id_cd, enregistrement.id_enr) GROUP BY id_interprete);

--Question 14

SELECT nom_pers AS Nom, prenom_pers AS prenom FROM personne INNER JOIN interprete ON id_interprete = id_pers GROUP BY nom_pers, prenom_pers HAVING COUNT(DISTINCT (id_cd, id_enr)) >= ALL(SELECT COUNT(DISTINCT (id_cd, id_enr)) FROM interprete GROUP BY id_interprete); 
